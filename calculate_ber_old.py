...
num_symbols = 100000  # Total number of symbols
M = 64 
P = 10
mu = 7
roll_off = 0.1
...
roll_off_index = utils.get_index_for_roll_off(roll_off)
...
utils.save_fft_data(fft_cur_pulse_rc,'', "RC", roll_off)
utils.save_fft_data(fft_cur_pulse_exp,'', "EXP", roll_off)
...
def process_file(file_path, bit_mapping, qam_symbols, h_data, b, x_real, x_imag, ISI_symbols_real, ISI_symbols_imag, ISI_channels, mu, device, ISI_contribution = 1):
    ...
    utils.save_fft_data(fft_cur_pulse, M_loss, "DNN", roll_off)
    ...
    plt.title(f'RC, EXP, and DNN Output (roll_off = {roll_off})')
    plt.xlabel('DFT Sample')
    plt.ylabel('Magnitude (FFT)')
    plt.legend()
    if ISI_contribution == 1:
        output_folder = './results/plots/with_ISI/'
    else:
        output_folder = './results/plots/without_ISI/'
